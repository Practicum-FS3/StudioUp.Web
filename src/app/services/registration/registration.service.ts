...
export class RegistrationService {
  ...
  getTrainings(): Observable<Training[]> {
    return this.http.get<any[]>(`${this.apiUrl}/Training`).pipe(shareReplay(1));
  }
  getPaymentOptions(): Observable<PaymentOption[]> {
    return this.http
      .get<any[]>(`${this.apiUrl}/PaymentOption`)
      .pipe(shareReplay(1));
  }
  getHMOs(): Observable<HMO[]> {
    return this.http.get<any[]>(`${this.apiUrl}/getAll`).pipe(shareReplay(1));
  }
  getSubscriptionTypes(): Observable<SubscriptionType[]> {
    return this.http
      .get<any[]>(`${this.apiUrl}/SubscriptionType`)
      .pipe(shareReplay(1));
  }
  getCustomerTypes(): Observable<CustomerType[]> {
    return this.http
      .get<any[]>(`${this.apiUrl}/CustomerType`)
      .pipe(shareReplay(1));
  }
}
